...
class _QuestViewState extends State<QuestView> {
    ...
    return Scaffold(
      body: Container(
        child: Consumer<QuestViewModel>(builder: (context, model, child) {
          ...
          final isLoading = model.getStatus() == ViewStatus.LOADING || quest == null;
          return Stack(
            children: <Widget>[
              isLoading
                            ...
                            Column(
                              children: <Widget>[
                                ...
                                ListTile(
                                    ...
                                    title: Text(
                                      "${quest.routines.length} days",
                                      style:
                                          TextStyle(color: AirnoteColors.grey),
                                    )),
                                ListTile(
                                    contentPadding:
                                    ...
                                    title: Text(
                                      "${quest.routines.map((routine) => routine.prompts.length).fold(0, (t, e) => t + e)} prompts",
                                      style:
                                          TextStyle(color: AirnoteColors.grey),
                                    )),
                                ListTile(
                                    ...
                                    title: Text(
                                      "${(quest.routines.map((routine) => routine.prompts.fold(0, (t, e) => t + e.duration)).fold(0, (t, e) => t + e) / 60000).round()} minutes",
                                      style:
                                          TextStyle(color: AirnoteColors.grey),
                                    )),
                              ],
              ...
              Align(
                ...
                child: Container(
                    ...
                    child: _getActionButton(quest, isLoading)),
              ),
            ],
          );
        }),
  ...
  Widget _getActionButton(Quest quest, bool isLoading) {
    if (isLoading)
      return CircularProgressIndicator(
        strokeWidth: 5,
        valueColor: AlwaysStoppedAnimation(AirnoteColors.backgroundColor),
      );
